...
const ConfirmList = ({ trips, fetch }) => {
  ...
  const cancelTrip = async (id, token) => {
    try {
        ...
        <>
          <div className="flex flex-col w-full mb-10">
            ...
            <div className="flex justify-between items-center ">
              <h1
                className="sm:text-3xl text-2xl font-medium leading-tight capitalize mb-4 text-green-900 cursor-pointer"
                onClick={() => setOpen(!open)}>
                Confirmed booking
              </h1>
              <KeyboardArrowDownIcon className="cursor-pointer w-28" onClick={() => setOpen(!open)} />
            </div>
              ...
              {trips.map((trip, _id) => (
                <>
                  <div className="p-4 lg:w-1/2 mb-10 md:mb-0 " key={_id}>
                    <div className="h-full md:flex sm:flex-row flex-col items-center sm:justify-start justify-center text-center sm:text-left">
                      <img
