...
const BookingList = () => {
  ...
  if(ratingDetails[0]){
    for(let i=1;i<=Math.floor(ratingDetails[0]?.average);i++){
      avgarray.push(i)
    }
    for(let j=1;j<=5;j++){
    if(ratingDetails[0]?.ratings[k]?.rating!=j){
  ratingArray.push({rating:j,percent:0})
     
      }
      else{
        ratingArray.push(ratingDetails[0]?.ratings[k])
        k++
      }
    }
  }
  ...
  const fetchHistory = async () => {
    ...
    if (response.status === 200)  {setTrips(response.data.Bookings)
...
<div class="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto pb-0">
      ...
      <div class="flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full rounded-[50px] text-center shadow-[1px_1px_2px_2px_grey]">
        ...
        <div class="mt-4 md:mt-6 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full">
          <div class="pb-4 md:pb-8 w-full md:w-40">
            <img class="w-full hidden md:block" src={`/images/${trips?.driver?.PicturePath}`} alt="profile" />
            <img class="w-full md:hidden" src={`/images/${trips?.driver?.PicturePath}`} alt="profile" />
          </div>
...
<p class="ml-2 text-lg font-medium text-bold text-gray-900 dark:text-white ">Driver Ratings</p>
</div>
<div class="flex items-center mb-5 justify-center w-50">

{avgarray?.map(avg=>    <svg aria-hidden="true" class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><title>Second star</title><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
)}
{ratingDetails[0]?.average%1!=0&&<svg aria-hidden="true" class="w-5 h-5 text-yellow-400" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
  <title>Half-colored star</title>
  <defs>
  ...
  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" fill="url(#halfStarGradient)"/>
    ...
    <p class="ml-2 text-sm font-medium text-gray-900 dark:text-white">{ratingDetails[0]?.average} out of 5</p>
    
</div>
{ratingArray.map(val=>
<div class="flex items-center mt-4 justify-center">
    <span class="text-sm font-medium text-blue-600 dark:text-blue-500">{val?.rating} star</span>
    <div class="w-2/4 h-5 mx-4 bg-gray-200 rounded dark:bg-gray-700">
        <div class="h-5 bg-yellow-400 rounded" style={{width: `${val?.percent}%`}}></div>
    </div>
    <span class="text-sm font-medium text-blue-600 dark:text-blue-500">{val?.percent}%</span>
</div>
)}
</div>:
<RideRating rating={rating} setRating={setRating} setTrips={setTrips} id={id} rated={trips?.rating}/>
}
    </>             
