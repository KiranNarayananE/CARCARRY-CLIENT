...
const PendingList = ({ trips, fetch }) => {
  ...
  const cancelBooking = async (token, tripId) => {
    try {
      const response = await axiosInstance.patch("/cancel-ride", { tripId }, { headers: { Authorization: `Bearer ${token}` } });
      return response;
    } catch (error) {
 ...
 const autoCancelPending = async (token) => {
    try {
      const response = await axiosInstance.delete("/auto-cancel", { headers: { Authorization: `Bearer ${token}` } });
      return response;
    } catch (error) {
      return error.response;
    }
